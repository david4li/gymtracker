...
import type { WeightUnit } from "@gymtracker/shared";
...
export function ActiveWorkoutClient({
  ...
  async function handleComplete(input: { weight: number; reps: number; rpe?: number; isWarmup: boolean }) {
    ...
    // The set is only added locally once the server confirms it. Appending optimistically
    // would silently desync this list from the database whenever a request fails, which on
    // gym wifi is a real scenario rather than a theoretical one.
    let row: Awaited<ReturnType<typeof logSet>>;
    try {
      setError(null);
      row = await logSet({
        workoutId,
      ...
      setError("Could not save that set. Check your connection and try again.");
      return;
    }
  ...
  async function handleDeleteSet(setId: number) {
    try {
      ...
      setError("Could not delete that set. Check your connection and try again.");
      return;
    }
  ...
  return (
    <div className="flex flex-col gap-4 pb-8">
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={() => {
              ...
              // Both of these navigate on success, so there is no "done" state to return to;
              // the pending flag exists to stop a second tap firing a duplicate request.
              startEnding(async () => {
                try {
                  await cancelActiveWorkout(workoutId);
                } catch {
                  setError("Could not discard the workout. Try again.");
                }
              });
            }}
          >
            ...
            onClick={() =>
              startEnding(async () => {
                try {
                  ...
                  setError("Could not finish the workout. Try again.");
                }
              })
            }
          >
      ...
      {error ? (
        <div
          role="alert"
          className="flex items-center justify-between gap-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700 dark:bg-red-950/40 dark:text-red-300"
        >
          <span>{error}</span>
